/**
 * The secure example's app factory — the single place all SEVEN security layers
 * are composed in front of the generated note router.
 *
 * `createSecureApp(deps)`:
 ...
 *
 * It is dependency-injected end to end: src/server.ts passes Redis stores + a real
 * OIDC verifier; the e2e test passes in-memory stores + a fake local-JWKS issuer.
 */
...
export interface CreateSecureAppDeps extends SecureExampleDeps {
...
export function createSecureApp(deps: CreateSecureAppDeps) {
