/**
 * Construction-time wiring for the secure example (ARCH-05).
 *
 ...
 *   - production       → Redis stores + a real remote OIDC verifier (src/server.ts)
 *   - the e2e test     → in-memory stores + a fake local-JWKS OIDC issuer (test/)
 *   - a dev boot       → Redis stores + (optionally) the same fake issuer
 */
...
/** The pluggable backends + collaborators the example's config is built from. */
export interface SecureExampleDeps {
  ...
  logger: Logger
  /** Injected audit sink (the adapter-node stdout sink in prod) — OPS-05. */
  audit: AuditSink
...
/**
 * Map verified OIDC claims → permission scopes (AUTH-08). This example reads a
 * space- or array-valued `scope`/`scp` claim (the common IdP conventions) and,
 * failing that, grants the base read/write scopes so the reference flow works
 * against a minimal IdP. A real deployment maps groups/roles to your scopes.
 */
...
export function buildPipelineConfig(deps: SecureExampleDeps): PipelineConfig {
  ...
  return {
    ...
    // Pluggable backends (ARCH-03) — REAL adapter stores in prod, fakes in tests.
    stores: {
