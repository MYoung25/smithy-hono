 ...
 * router's `all` slot (see src/server.ts) runs `withPrincipal(...)` so the ops
 ...
 * already established. (Node-only — `node:async_hooks`; this example is the
 * adapter-node deployment, so that is fine.)
 */
