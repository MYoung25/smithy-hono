...
/**
 * The secure-api SPA. One {@link createBrowserSession} drives the whole OIDC
 ...
 *
 * NOTE on paths: secure-api mounts the CSRF-token route at `/csrf-token` (not the
 ...
 * (the Cloudflare full-stack deploy mounts the service at `/api`), the build sets
...
function makeSession(onChange: (status: AuthStatus) => void) {
  return createBrowserSession({
    ...
    // secure-api's csrfTokenHandler is mounted at `${API_BASE}/csrf-token`, outside /auth.
