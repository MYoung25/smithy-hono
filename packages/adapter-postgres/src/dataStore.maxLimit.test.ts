 ...
 * Postgres DataStore configured with `maxLimit` silently CLAMPS a `list`'s
...
async function seed(store: ReturnType<typeof createPostgresDataStore<Row>>, n: number) {
  for (let i = 0; i < n; i++) {
...
describe('PostgresDataStore — maxLimit clamp', () => {
  it('clamps a large limit to maxLimit and returns a valid resume cursor', async () => {
    const store = createPostgresDataStore<Row>(createFakePgDataPort(), { maxLimit: 5 })
    await seed(store, 20)
