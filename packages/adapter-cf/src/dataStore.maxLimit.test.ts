/**
 * Focused regression for the optional `maxLimit` clamp (defense-in-depth) on the
 * KV DataStore: a store configured with `maxLimit` silently CLAMPS a `list`'s
 * effective page size to it and still emits a valid resume cursor, while an UNSET
 * `maxLimit` leaves a large `limit` untouched (backwards-compatible default).
 */

import { describe, it, expect } from 'vitest'
import { createKvDataStore } from './dataStore.js'
...
async function seed(store: ReturnType<typeof createKvDataStore<Row>>, n: number) {
  for (let i = 0; i < n; i++) {
...
describe('KvDataStore — maxLimit clamp', () => {
  it('clamps a large limit to maxLimit and returns a valid resume cursor', async () => {
    const store = createKvDataStore<Row>(new FakeKvNamespace(), { maxLimit: 5 })
    await seed(store, 20)
  ...
  it('UNSET maxLimit leaves a large limit unchanged (backwards-compatible)', async () => {
    const store = createKvDataStore<Row>(new FakeKvNamespace())
    await seed(store, 20)
    const page = await store.list({ limit: 1000 }, SCOPE)
