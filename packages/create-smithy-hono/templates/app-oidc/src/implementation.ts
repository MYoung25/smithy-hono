 ...
 * src/createApp.ts. These handlers assume they only ever run for a request that
 * has already passed authentication, authorization, CSRF, and (for ImportNotes)
 * S2S signature verification. They read the caller's identity via the
 * request-scoped {@link requirePrincipal} (populated by the `all`-slot middleware
 * in createApp.ts).
 */
...
export function createNoteOps(store: NotesStore): NoteOperations {
  return {
    ...
    async GetNote({ id }) {
      // The isOwner resource policy (createApp.ts) already enforced ownership; we
      // re-read from the store for a self-contained handler. A 404 here is
      // unreachable in practice (the policy 404s a missing note first) but kept
