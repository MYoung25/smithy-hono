 ...
 * Redis DataStore configured with `maxLimit` silently CLAMPS a `list`'s effective
 * page size to it and still emits a valid resume cursor, while an UNSET `maxLimit`
 * leaves a large `limit` untouched (backwards-compatible default).
 */

import { describe, it, expect } from 'vitest'
import { createRedisDataStore, createFakeRedisDataPort } from './dataStore.js'
...
async function seed(store: ReturnType<typeof createRedisDataStore<Row>>, n: number) {
  for (let i = 0; i < n; i++) {
...
describe('RedisDataStore — maxLimit clamp', () => {
  it('clamps a large limit to maxLimit and returns a valid resume cursor', async () => {
    const store = createRedisDataStore<Row>(createFakeRedisDataPort(), { maxLimit: 5 })
    await seed(store, 20)
  ...
  it('UNSET maxLimit leaves a large limit unchanged (backwards-compatible)', async () => {
    const store = createRedisDataStore<Row>(createFakeRedisDataPort())
    await seed(store, 20)
    const page = await store.list({ limit: 1000 }, SCOPE)
