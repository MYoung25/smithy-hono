...
> **One-command deploy CLIs.** Node and AWS now join Cloudflare with config-driven
> one-command deploy CLIs (`@smithy-hono/deploy-node`, `@smithy-hono/deploy-aws`,
> `@smithy-hono/deploy-cf`) that take a project from a domain to a live UI + API
> same-origin — see [`scaffolding.md`](./scaffolding.md). This page documents the
> underlying manual/reference wiring and the per-platform gotchas.
